...
class CellularAutomataGeneration:
    ...
    def __init__(self, width = 200, height = 50, percent_fill = 0.48, border_thickness = 5, smooth_passes = 5, border = True):
        self.width = width
        self.height = height
        ...
        self.array = [[random.uniform(0.0, 1.0) < self.percent_fill for x in range(self.width)] for y in range(self.height)]
    ...
    def smooth(self):
        ...
        def get_neighbor_count(x, y) -> int:
            ...
            for dx in range(-1, 2):
                for dy in range(-1, 2):
                    count += 1 if (in_range(x + dx, y + dy) and copy[y + dy][x + dx]) or not in_range(x + dx, y + dy) else 0  
            return count
    ...
    def add_border(self):
        ...
        for i, row in enumerate(self.array):
            self.array[i] = [True] * self.border_thickness + row + [True] * self.border_thickness
            
        self.array = self.border_thickness * [[True] * len(self.array[0])] + self.array + [[True] * len(self.array[0])] * self.border_thickness
